...
import { Plus, Package, ToggleLeft, ToggleRight, CheckCircle, XCircle } from "lucide-react";
...
function VendorDashboard() {
  ...
  const toggleDropStatus = async (itemId, currentStatus) => {
    try {
      await axios.put(`${API}/vendor/drops/${itemId}`, { is_active: !currentStatus }, { withCredentials: true });
  ...
  const updateOrderStatus = async (orderId, status) => {
    try {
      await axios.put(`${API}/vendor/orders/${orderId}/status`, { status }, { withCredentials: true });
        ...
        <div className="flex gap-6">
          <button
            ...
            className={`py-3 border-b-2 font-medium transition-colors ${
              activeTab === "drops" ? "border-[#2E7D32] text-[#2E7D32]" : "border-transparent text-[#4A5D4E]"
            }`}
          ...
          <button
            ...
            className={`py-3 border-b-2 font-medium transition-colors ${
              activeTab === "orders" ? "border-[#2E7D32] text-[#2E7D32]" : "border-transparent text-[#4A5D4E]"
            }`}
        ...
        {loading ? (
          ...
          drops.length === 0 ? (
            <div className="text-center py-20">
              ...
              <Button onClick={() => navigate("/vendor/add-drop")} className="bg-[#2E7D32] hover:bg-[#205a22] text-white rounded-xl px-6 py-2">
                Add Drop
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              {drops.map((drop) => (
                <div key={drop.item_id} className="bg-white border border-[#E5E2DA] rounded-2xl overflow-hidden" data-testid="vendor-drop-card">
                  <div className="flex gap-4 p-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <button onClick={() => toggleDropStatus(drop.item_id, drop.is_active)} data-testid="toggle-drop-status">
                          {drop.is_active ? <ToggleRight size={32} className="text-[#2E7D32]" /> : <ToggleLeft size={32} className="text-[#4A5D4E]" />}
                        </button>
                      </div>
                      <div className="flex items-center gap-4 text-sm text-[#4A5D4E] mb-3">
                        <span>{drop.quantity_available} left</span>
                        <span className={`px-2 py-0.5 rounded-full text-xs ${drop.is_active ? "bg-[#2E7D32]/10 text-[#2E7D32]" : "bg-gray-100 text-gray-600"}`}>
                          {drop.is_active ? "Active" : "Inactive"}
                        </span>
                      </div>
                      <button onClick={() => navigate(`/vendor/edit-drop/${drop.item_id}`)} className="text-sm text-[#2E7D32] hover:underline" data-testid="edit-drop-button">
                        Edit
                      </button>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <div key={order.order_id} className="bg-white border border-[#E5E2DA] rounded-2xl p-4" data-testid="vendor-order-card">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-lg font-medium text-[#2E7D32]">&#8377;{order.total_price.toFixed(2)}</div>
                  ...
                  {order.status === "reserved" && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-[#2E7D32] text-white text-xs font-medium rounded-lg hover:bg-[#205a22] transition-colors active:scale-[0.97]"
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 border border-[#C65D47] text-[#C65D47] text-xs font-medium rounded-lg hover:bg-[#C65D47]/10 transition-colors active:scale-[0.97]"
                ...
                {order.pickup_time && (
                  <p className="text-xs text-[#4A5D4E] mt-2">Pickup: {order.pickup_time}</p>
                )}
              </div>
            ))}
          </div>
